<div>
    ...
    <x-admin.table title="All Jobs" icon="folder-plus" :route="route('jobs.create')"  :columns="['name', 'الدولة - المدينة', 'الفئة', 'الزيارات', 'الحالة', 'تاريح الاظافة', 'actions']">
            ...
            @forelse($jobs as $job)
                <tr>
                    <td title="{{ $job->name }}"> @if(strlen($job->name) > 60) {{ substr($job->name, 0, 60) }} ... @else {{ $job->name }} @endif </td>
                    ...
                    <td>
                        <div class="d-flex align-items-center">
                            <a href="{{ route('web.jobs.show', $job) }}" target="_blank" style="font-size: 1.2rem" class="mdi mdi-eye text-primary ms-2"></a>
