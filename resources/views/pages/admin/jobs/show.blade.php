                ...
                <div class="d-flex">
                  {{-- <div class="d-flex align-items-center me-4 text-muted font-weight-light">
                    ...
                    <div style="width: 300px">
                        <img src="{{ asset('storage/images/jobs/'. $job->image) }}" class="mb-2 mw-100 w-100 rounded" alt="image">
                    </div>
                    ...
                    <div class="row">
                        <form action="{{ route('posts.keyword', $job->id) }}" method="POST">
                            ...
                            <x-admin.text-area placeholder="keywords" name="keywords" rows="10">
                                وظائف في مدينة {{ $job->city->name }}, وظائف في دولة {{ $job->country->name }}, وظائف {{ $job->city->name }}, {{ $job->category->name }},
                            </x-admin.text-area>


                            <div class="col-md-4 form-group mt-4 mb-2">
                                <div class="d-flex align-items-center">
                                    <input class="form-check-input" type="checkbox" value="{{ App\Http\Helpers\PostStatus::PUBLISH->value }}" @checked($job->status == App\Http\Helpers\PostStatus::PUBLISH->value) id="status" name="status">
                                    <label class="mb-0 fw-bold me-2"> نشر الوظيفة </label>
                                </div>
                            </div>

                            <x-admin.button class="btn-gradient-primary btn-fw mt-3">{{ __('Save') }}</x-admin.button>
