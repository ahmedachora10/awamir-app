<x-main-layout>
        ...
        <center>
            <form action="/jobs" method="get" >
                <div style="position: relative;width:95%" class="mx-auto">
                    <div class="search">
                        <input type="search" name="squery" class="squery" placeholder="ابحث عن الوظيفة المناسبة "/><button type="submit" ><i class="bi bi-search"></i></button>
                    </div>
    ...
    <div class="main2 mt-4">
      <center>
        <h2>تصفح الوظائف</h2>
        <div class="cat cat1 cat-act"><div class='ico'><i class="bi bi-clock-fill"></i></div> أحدث الوظائف </div>
        <div class="cat cat2"><div class='ico'><i class="bi bi-lightning-charge-fill"></i></div> أهم الوظائف</div>
        <div class="new-job">
    ...
    <div class="main3">
        <center>
            ...
            @foreach ($categories as $category)
                <a href="{{ route('web.jobs.index') }}?category={{$category->id}}"><div class="categorie">{{$category->name}}</div></a>
            @endforeach
        </center>
    </div>
    <div class="main4">
                        ...
                        console.log(data);
                            if (squery == '') {
                                ...
                                if (!data.length){
                                    ...
                                    result += '<div class="search-q" onclick="show(this.innerHTML)">لا شيء مطابق</div>' ;
                                    ...
                                    for(let i = 0; i < length ; i++){
                                        var dd = data[i].name;
                                        result += '<div class="search-q" onclick="show(this)" id="'+dd.substring(0,15)+'"><i class="bi bi-search"></i>'+dd.substring(0,15)+' ...</div> ' ;
                                        result2 +=     '<div class="search-c" id="'+data[i].category.id+'" onclick="show2(this)">'+data[i].category.name+'</div>' ;
                                    }
                                    result2 += '</div>';
